...
public class ClsAgentUpdateBulletinThread
{
	...
	public int nbj_enf = 0;

	public int nbj_anc = 0;
	...
	private List<CalculPaie> listeRhtCalculs = new ArrayList<CalculPaie>();
	...
	private ClsUpdateBulletinThread update;
	...
	public ClsAgentUpdateBulletinThread(ClsUpdateBulletinThread update, Session oSession, Salarie salarie)
	{
		this.update = update;
...
//					update_flag = true;
				...
				int count = listeRhtCalculs.size();
				if (count == 0)
				{
...
//				retour = deleteEFMois(salarie);
				...
				{
//					oSession.createQuery("delete from Rhtfic" + " where cdos = '" + update.dossier + "'" + " and nmat = '" + salarie.getNmat() + "'" + " and nbul = " + update.numerobulletin).executeUpdate();
					sup_fic_flag = true;
				}
			}
		}
	...
	private boolean updateCongePayeNonPris(Salarie salarie, BigDecimal mont)
	{
		...
		else
		{
			...
			nbjstot = salarie.getNbjse().intValue() + salarie.getNbjsa().intValue() + salarie.getNbjsm().intValue();
			if (wnbjcad >= nbjstot)
			{
				salarie.setNbjse(new BigDecimal(0));
				salarie.setNbjsa(new BigDecimal(0));
			...
			{
				wnbjcad2 = nbjstot;
				if (wnbjcad2 >= salarie.getNbjsa().intValue())
				{
					wnbjcad2 -= salarie.getNbjsa().intValue();
					salarie.setNbjsa(new BigDecimal(0));
					if (wnbjcad2 >= salarie.getNbjse().intValue())
					{
						wnbjcad2 -= salarie.getNbjse().intValue();
						salarie.setNbjse(new BigDecimal(0));
						if (wnbjcad2 >= salarie.getNbjsm().intValue())
						{
							salarie.setNbjsm(new BigDecimal(0));
						}
						else
						{
							salarie.setNbjsm(new BigDecimal(salarie.getNbjsm().intValue() - wnbjcad2));
						}
					}
					else
					{
						salarie.setNbjse(new BigDecimal(salarie.getNbjse().intValue() - wnbjcad2));
					}
				}
				else
				{
					salarie.setNbjsa(new BigDecimal(salarie.getNbjsa().intValue() - wnbjcad2));
				}
			}
			if (wnbjcad <= salarie.getJapa().intValue())
			{
				salarie.setJapa(new BigDecimal(salarie.getJapa().intValue() - wnbjcad));
			}
			else
			{
				salarie.setJapec(new BigDecimal(salarie.getJapec().intValue() - (wnbjcad - salarie.getJapa().intValue())));
				salarie.setJapa(new BigDecimal(0));
			}

			salarie.setJrlec(new BigDecimal(salarie.getJrlec().intValue() + wnbjcad));
	...
	private boolean updatePret13(String nmat, int lg, String crub, BigDecimal mont)
	{
		...
		Object o = null;
		String queryString = "From PretInterne Where idEntreprise="+update.dossier+" "+
				" And nmat='"+nmat+"' And lg="+lg;
		Session session = update.service.getSession();
		Query query = session.createSQLQuery(queryString);
		List result = query.list();
		update.service.closeSession(session);
		if(result!=null && result.size()>0) o = result.get(0);
	...
	private boolean genElementVariableNextMonth(String nmat, String crub, BigDecimal mont)
	{
		...
		CleEltVarDet pkEvDet = new CleEltVarDet(evar.getIdEntreprise(), evar.getAamm(), evar.getNmat(), evar.getNbul());

		ElementVariableEnteteMois o = null;
		if(! maj_eva_ent_already_check)
		{
		 //o = (Rhteltvarent) update.service.get(Rhteltvarent.class, new CleEltVarEnt(evar.getCdos(), evar.getAamm(), evar.getNmat(), evar.getNbul()));
		 List<ElementVariableEnteteMois> result = update.service.find("FROM ElementVariableEnteteMois Where idEntreprise="+evar.getIdEntreprise()+" And aamm='"+evar.getAamm()+"' "+
		 																	"And nmat='"+evar.getNmat()+"' And nbul="+evar.getNbul().intValue());
		 if (result != null && result.size()>0) o = result.get(0);
		...
		if(!mapRhteltvardetDejaGenere.containsKey(pkEvDet))
		{
			oSession.save(evar);
			mapRhteltvardetDejaGenere.put(pkEvDet, pkEvDet);
		}
	...
	private void _chargerCumuls(String nmat)
	{
		...
		for (CumulPaie cumul : liste)
		{
			CleCumulPaie cleCumulPaie = new CleCumulPaie(cumul.getIdEntreprise(), cumul.getNmat(), cumul.getAamm(), cumul.getRubq(), cumul.getNbul());
			this.cumulsMap.put(cleCumulPaie, new ClsCumul(cumul, true));
		}
		//System.out.println("Aprés chargement des cumuls du salarié "+nmat+", taille du map = "+this.cumulsMap.size());
	...
	private void _chargerCumuls99(String nmat)
	{
		...
		//System.out.println("Aprés chargement des cumuls 99 du salarié "+nmat+", taille du map = "+this.cumulsMap.size());
	...
	private boolean lectureCalculSurMoisCourant(Salarie salarie)
	{
		...
		listeRhtCalculs = update.updateservice.getListeBulletinCalculFromRhtcalcul(update.dossier, update.periode, update.numerobulletin, nmat);
		...
		for (CalculPaie ocalcul : listeRhtCalculs)
		{
...
//						}
	...
	private boolean transferEcheancePretMoisSuivant(String nmat)
	{
		...
		{
			String queryString = "From PretExterneDetail Where idEntreprise="+update.dossier+" And nprt='"+entete.getNprt()+"' And perb=:perb";
			Session session = update.service.getSession();
			Query query = session.createSQLQuery(queryString);
			query.setParameter("perb", new ClsDate(update.periode, ClsParameterOfPay.FORMAT_DATE_PAY_PERIOD_YYYYMM).getDate(), StandardBasicTypes.DATE);
			List<PretExterneDetail> result = query.list();
			update.service.closeSession(session);
			if(result!=null && result.size()>0) o = result.get(0);
	...
	private boolean transferEVMoisSuivant_save(String nmat)
	{
		...
		return update.updateservice.transferEVMoisSuivant(oSession, update.dossier, update.periode, update.numerobulletin, update.mois_ms, update.nbul_ms, nmat, new ClsDate(date_debut_mois_ms).getDateS(update.dateformat), new ClsDate(date_fin_mois_ms).getDateS(update.dateformat), update.user);
	}
	...
	private boolean transferEVMoisSuivant(String nmat)
	{
		...
		for (Object object : l)
		{
			...
			String queryString = "From ElementVariableEnteteMois Where idEntreprise="+evar.getIdEntreprise()+" And aamm='"+evar.getAamm()+"' "+
					             " And nmat='"+evar.getNmat()+"' And nbul='"+evar.getNbul()+"'";
			Session session = update.service.getSession();
			Query query = session.createSQLQuery(queryString);
			List<ElementVariableEnteteMois> result = query.list();
			update.service.closeSession(session);
			if(result!=null && result.size()>0) o = result.get(0);
			//o = update.service.get(Rhteltvarent.class, new CleEltVarEnt(evar.getCdos(), , evar.getNmat(), evar.getNbul()));
			if (o == null)
			{
	...
	private boolean chargerEVCGMoisCourant(String nmat)
	{
		...
		ElementVariableEnteteMois event = null;
		ClsUpdateBulletinThread.RhfnomMotifConge fnomMotif = null;
		...
		for (Object[] obj : l)
		{
			...
			if(fnomMotif != null)
			{
				...
				{
					nbjreliq += evconge.getNbja().intValue();
				}

				if (fnomMotif.montant1 == 1 && fnomMotif.montant3 == 1)
				{
					nbjcp += evconge.getNbjc().intValue();
				}
			...
			elementConge.nbja = evconge.getNbja().intValue();
			elementConge.nbjc = evconge.getNbjc().intValue();
	...
	private boolean transfererUnEVCGMoisSuivant(DetailEv detailEv ,String nmat)
	{
		...
		try
		{
			...
			String queryString = "From ElementVariableEnteteMois Where idEntreprise="+evconge.getIdEntreprise()+" And aamm='"+evconge.getAamm()+"' "+
					" And nmat='"+evconge.getNmat()+"' And nbul='"+evconge.getNbul()+"'";
			Session session = update.service.getSession();
			Query query = session.createSQLQuery(queryString);
			List<ElementVariableEnteteMois> result = query.list();
			update.service.closeSession(session);
			if(result!=null && result.size()>0) eventete = result.get(0);
			//eventete = (Rhteltvarent) update.service.get(Rhteltvarent.class, new CleEltVarEnt(evconge.getCdos(), evconge.getAamm(), evconge.getNmat(), evconge.getNbul()));
			if (eventete == null)
			{
				ElementVariableEnteteMois event = new ElementVariableEnteteMois();
	...
	public int  getIndexDernierMoisClotureDansListOfPeriodeT91()
	{
		...
		{
			if(StringUtil.equals(update.listePeriodesT91.get(i).periode, update.periode))
					return i;
		}
		return -1;
	}
...
//							}
	...
	private boolean insertCongeMoisSuivant(DetailEv infoperiode, String nmat)
	{
		...
		update.service.closeSession(session);
		if(result!=null && result.size()>0) o = result.get(0);
		//Object o = update.service.get(Rhteltvarent.class, new CleEltVarEnt(evconge.getCdos(), evconge.getAamm(), evconge.getNmat(), evconge.getNbul()));
		if (o == null)
		{
		...
		//
		queryString = "From ElementVariableConge Where idEntreprise="+evconge.getIdEntreprise()+" And aamm='"+evconge.getAamm()+"' "+
				" And nmat='"+evconge.getNmat()+"' And nbul='"+evconge.getNbul()+"' And ddeb=:ddeb";
		session = update.service.getSession();
		query = session.createSQLQuery(queryString);
		query.setParameter("ddeb", evconge.getDdeb(), StandardBasicTypes.DATE);
		List<ElementVariableConge> result2 = query.list();
		update.service.closeSession(session);
		if(result2!=null && result2.size()>0) o = result2.get(0);
...
//			salarie.update_flag = true
		...
		else
		{
			...
			{
				if (salarie.getNbjtr().add(new BigDecimal(nbjtr)).intValue() < 999)
				{
...
//		update_flag = true;
	...
	private boolean calculateNbreJourSupplementaire(Salarie salarie)
	{
		...
		try
		{
			...
			ClsUpdateBulletinThread.RhfnomCalcPartFiscal fnom = null;
...
//							if (ev.dfin.equals(salarie.getDfcf()) && conges_annuels)
	...
	private boolean updateCongePayePonctuel(DetailEv ev, Salarie salarie)
	{
		...
		{
			int i = salarie.getJapa().intValue();
			if (salarie.getJapec() != null)
	...
	private boolean calculPartFiscale(Salarie salarie)
	{
		...
		ClsUpdateBulletinThread.RhfnomNbParts fnom = update.getRhfnomNbParts(salarie.getSitf());
		double nbpt = 1;
		nbpt += fnom.taux1;
		...
		if (nbpt != salarie.getNbpt().intValue())
		{
...
//			update_flag = true;
	...
	public void agentUpdateToString()
	{
		...
		ClsParameter.println(StringUtil.toString(salarie));
	...
	*/
	...
	public int getNbj_enf()
	{
		return nbj_enf;
	}

	public int getNbj_anc()
	{
		return nbj_anc;
	}
	...
	public List<CalculPaie> getListeRhtCalculs()
	{
		return listeRhtCalculs;
	}
