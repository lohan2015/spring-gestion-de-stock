...
public class ClsStringUtil {
	...
	public static String replace(String chaine, char oldChar, char newChar)
	{
		if(StringUtils.isBlank(chaine)) return chaine;
		return chaine.replace(oldChar, newChar);
	}
	
	public static String oraLPad(String chaine, int totalLength, String charToAdd)
	{
		String add="";
		if(StringUtils.isBlank(chaine)) chaine="";
		if(chaine.length()<totalLength)
			for (int i = 0; i < totalLength-chaine.length(); i++) add += charToAdd;
		return add + chaine;
	}
	...
	public static String oraRPad(String chaine, int totalLength, char charToAdd)
	{
		String add="";
		if(StringUtils.isBlank(chaine)) chaine="";
		if(chaine.length()<totalLength)
...
//		 The 0 symbol shows a digit or 0 if no digit present
	...
	public static String reviewSubstring(String in, int minIdx, int strLen)throws Exception, StringIndexOutOfBoundsException{
		if(1 == 1)
			return StringUtils.substring(in, minIdx, minIdx + strLen);
//		 The 0 symbol shows a digit or 0 if no digit present
		if(strLen < 0 || (minIdx + strLen > in.length()))
			throw new StringIndexOutOfBoundsException("La taille sp�cifi�e est incorrecte.");
		
		if(minIdx >= 0 )
			return in.substring(minIdx, minIdx + strLen);
		else{		
			int len = in.length();
			int iMinIdx = Math.abs(minIdx);
			if(iMinIdx > len || iMinIdx < strLen)
				throw new Exception("Cannot get the length specified.");
			int idx = len - iMinIdx;
			return in.substring(idx, idx + strLen);
		}
	}
