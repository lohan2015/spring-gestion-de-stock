...
public class ClsOrgListCellsDesigner implements ICellsDesigner
{
	...
	public ClsOrgListCellsDesigner(GeneriqueConnexionService service, ClsParametreOrganigrammeVO param,ClsMessageCelluleVO message)
	{
	...
	public String _drawFlowChart()
	{
		...
		for (int i = 0; i < cellulePere.getFils().size(); i++)
		{
			...
			if (celluleFille.getNbrFils() > 0)
			{
				...
				_strOrganigramme = _strOrganigramme.replace(ClsTemplateGenerator.STR_FILS + celluleFille.getCodeorganigramme(), ClsTemplateGenerator._getTemplate(celluleFille,
						allCellules));
